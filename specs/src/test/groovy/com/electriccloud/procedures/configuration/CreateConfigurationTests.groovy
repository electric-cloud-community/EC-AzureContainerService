...
class CreateConfigurationTests extends AzureTestBase {
    ...
    void createConfigurationWithTestConnection() {
        ...
        def logs = acsClient.client.getJobLogs(job.json.jobId)
    ...
    @Test(testName = "Create config without test connection")
    @TmsLink("")
    ...
    @Test(dataProvider = "logLevels",
            testName = "Create config with LogLevels",
    ...
    void createConfigurationForDifferentLogLevels(logLevel, message, desiredLog, missingLog){
        ...
        def jobStatus = acsClient.client.getJobStatus(job.json.jobId).json
        def jobSteps = acsClient.client.getJobSteps(job.json.jobId).json.object
        def logs = acsClient.client.getJobLogs(resp.json.jobId)
    ...
    @Test(dataProvider = "configData", testName = "Create config with parameters validation")
    @TmsLink("")
    ...
    @Test(testName = "Create config that allready exist")
    @TmsLink("")
    ...
    @Test(dataProvider = "invalidData",
            testName = "Create config with invalid data")
    @Story("Invalid configuration")
    @TmsLink("")
