...
class ClusterView {
    ...
    private static final String DISPLAY_CLUSTER = 'ACS Cluster'
    private static final String DISPLAY_NAMESPACE = 'ACS Namespace'
    private static final String DISPLAY_POD = 'ACS Pod'
    private static final String DISPLAY_CONTAINER = 'ACS Container'
    private static final String DISPLAY_SERVICE = 'ACS Service'
    ...
    def getContainerDetails(String containerName) {
        ...
        try {
            ...
            if (e.message =~ /404/) {
                node.addAttribute(ATTRIBUTE_ERROR, "ACS pod '${podId}' that the container '${containerId}' belonged to does not exist in namespace '${namespace}'", TYPE_STRING)
                return node
            }
            else {
                throw e
...
//        So let's poke them all, maybe we are lucky
    ...
    def getNamespaceDetails(String namespaceId) {
        ...
        try {
            ...
            if (e.message =~ /404/) {
                node.addAttribute(ATTRIBUTE_ERROR, "ACS namespace '${namespaceName}' does not exist", TYPE_STRING)
                return node
            }
            else {
                throw e
    ...
    def getServiceDetails(serviceName) {
        ...
        try {
            ...
            if (e.message =~ /404/) {
                node.addAttribute(ATTRIBUTE_ERROR, "ACS service '${serviceId}' does not exist in namespace '${namespace}'", TYPE_STRING)
                return node
            }
            else {
                throw e
