...
@Builder(builderStrategy = ExternalStrategy, forClass = Discovery, excludes = 'azureClient, accessToken, clusterEndpoint, discoveredSummary')
...
//    Target
    ...
    def discover() {
        def kubeServices = azureClient.getServices(clusterEndpoint, namespace, accessToken)
        def efServices = []
        kubeServices?.items.each { kubeService ->
            if (!isSystemService(kubeService)) {
                ...
                def deployments = azureClient.getDeployments(
                    clusterEndpoint,
...
//                items += (deploymentConfigs?.items ?: [])
    ...
    def buildSecretsDefinition(namespace, secrets) {
        ...
        secrets.each {
            def name = it.name
            def secret = azureClient.getSecret(name, clusterEndpoint, namespace, accessToken)
